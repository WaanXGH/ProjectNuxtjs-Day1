...
<template>
    <section class="bg-white shadow-xl rounded-xl overflow-hidden">
        ...
        <div class="px-5 pb-5 pt-9 relative">
            ...
            <NuxtLink :to="`/product/${oneProduct.id}`">
                <h3 class="text-lg font-bold mb-4 text-limit limit-2">{{ oneProduct.name }}</h3>
            </NuxtLink>
    ...
    ${oneProduct.isCart ? 'bg-blue-600 text-white' : 'bg-white'} shadow-xl rounded-full flex justify-center items-center hover:bg-blue-600 hover:text-white transition duration-300`"
                @click="addCart">
